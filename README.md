...
user says something that means "no, that's not it"
        │
   [1] RESTATE  ─ say their real intent back in one line, before anything else.
        │         At most one clarifying question.
   [2] SEARCH   ─ scan both INDEX.md files; open only candidate S files. active →
        │         apply + cite the ID; provisional → same, disclosed. Re-corrected
        │         after applying one? → counterexample F, and that S is demoted.
   [3] FIX      ─ carry out the correction. Recording never blocks the work.
   [4] CAPTURE  ─ write <store>/patterns/F-<date>-<rand8>.md, refresh INDEX (+ commit).
   [5] TRIAGE   ─ rate context health, then recommend — advice only: 1st correction
        │         → continue · 2nd on the same item → rewind · repeated failure →
        │         "F-<id> recorded. New session, read only that file, retry."
        │
   compile ─┬─ automatic: evidence E ≥ 5 → provisional S (usable now, disclosed)
